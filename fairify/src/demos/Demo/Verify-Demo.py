...
for model_file in model_files:
    ...
    for i in range(len(model.layers)):
        all_weights.append(model.layers[i].get_weights())
        w.append(model.layers[i].get_weights()[0])
        b.append(model.layers[i].get_weights()[1])
        
    
    fig, axs = plt.subplots(len(all_weights), 1, figsize=(8, 2 * len(all_weights)))

    for i, weights in enumerate(all_weights):
        flat_weights = np.concatenate([w.flatten() for w in weights])
        axs[i].hist(flat_weights, bins=20, color='blue', alpha=0.7)
        axs[i].set_title(f'Layer {i + 1} Weights Distribution')

    plt.tight_layout()

    # 保存图片到当前目录
    plt.savefig('all_weights_distribution.png')
    ...
    #    sd = s
        ...
        fig, axs = plt.subplots(len(all_weights), 1, figsize=(8, 2 * len(all_weights)))
        for i, weights in enumerate(pr_w):
            flat_weights = np.concatenate([w.flatten() for w in weights])
            axs[i].hist(flat_weights, bins=20, color='blue', alpha=0.7)
            axs[i].set_title(f'Layer {i + 1} Weights Distribution')
        plt.tight_layout()
        ...
        # 保存图片到当前目录
        plt.savefig('prune_weights_distribution.png')
        ...
        if(len(sys.argv) > 1):
            ...
            s.set("timeout", SOFT_TIMEOUT * 1000 * 5)
