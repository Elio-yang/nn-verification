...
df, X_train, y_train, X_test, y_test = load_deposit()
...
for model_file in model_files:
    ...
    #    sd = s
        neuron_bounds, candidates, s_candidates, b_deads, s_deads, st_deads, pos_prob, sim_X_df  = \
            sound_prune_deposit(df, w, b, simulation_size, layer_net, p)
        ...
        for attr in A:
            if(attr in PA):
                in_props.extend(in_const_deposit(df, x, attr, 'neq', x_))
            elif(attr in RA):
                in_props.extend(in_const_diff_deposit(df, x, x_, attr, RA_threshold))
            else:
                in_props.extend(in_const_deposit(df, x, attr, 'eq', x_))
    
        in_props.extend(in_const_domain_deposit(df, x, x_, p, PA))
