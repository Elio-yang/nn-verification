...
df, X_train, y_train, X_test, y_test = load_fraud()
X = np.r_[X_train, X_test]

#print('shape ', X_train[0].shape)

single_input = X_test[0].reshape(1, 29)
...
range_dict['V3']=[-48,10]
range_dict['V4']=[-5,17]
range_dict['V5']=[-114,35]
range_dict['V6']=[-27,74]
range_dict['V7']=[-44,121]
range_dict['V8']=[-74,21]
range_dict['V9']=[-14,16]
range_dict['V10']=[-25,24]
range_dict['V11']=[-5,13]
range_dict['V12']=[-19,8]
range_dict['V13']=[-5,8]
range_dict['V14']=[-19,11]
range_dict['V15']=[-5,9]
range_dict['V16']=[-15,18]
range_dict['V17']=[-26,10]
range_dict['V18']=[-10,6]
range_dict['V19']=[-8,6]
range_dict['V20']=[-55,40]
range_dict['V21']=[-35,28]
range_dict['V22']=[-11,11]
range_dict['V23']=[-45,23]
range_dict['V24']=[-3,5]
range_dict['V25']=[-11,8]
range_dict['V26']=[-3,4]
range_dict['V27']=[-23,32]
range_dict['V28']=[-16,34]
range_dict['Amount']=[0,25692]
...
for model_file in model_files:
    ...
    #    sd = s
        neuron_bounds, candidates, s_candidates, b_deads, s_deads, st_deads, pos_prob, sim_X_df  = \
            sound_prune_fraud(df, w, b, simulation_size, layer_net, p)
        ...
        x = np.array([Int('x%s' % i) for i in range(29)])
        x_ = np.array([Int('x_%s' % i) for i in range(29)])
        ...
        for attr in A:
            if(attr in PA):
                in_props.extend(in_const_fraud(df, x, attr, 'neq', x_))
            elif(attr in RA):
                in_props.extend(in_const_diff_fraud(df, x, x_, attr, RA_threshold))
            else:
                in_props.extend(in_const_fraud(df, x, attr, 'eq', x_))
    
        in_props.extend(in_const_domain_fraud(df, x, x_, p, PA))
