...
def net(x, w, b):
    ...
    x1 = w[0].T @ x + b[0]
    y1 = leaky_relu(x1,default_alpha)

    x2 = w[1].T @ y1 + b[1]
    y2 = leaky_relu(x2,default_alpha)
    ...
    # y = 1 / (1 + math.exp(-x8)) # WP computer for sigmoid
    return x8
...
def z3_net(x, w, b):
    ...
    x1 = w[0].T @ fl_x + b[0]
    y1 = z3LeakyRelu(x1,default_alpha)
    
    x2 = w[1].T @ y1 + b[1]
    y2 = z3LeakyRelu(x2,default_alpha)
